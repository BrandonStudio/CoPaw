...
class WebhookManager:
    ...
    async def create_subscription(self) -> bool:
        ...
        """
        logger.info("Creating webhook subscription for %s", self.notification_url)
        ...
        if result and "id" in result:
            ...
            if expiration_str:
                try:
                    ...
                    logger.warning("Failed to parse expiration time: %s", expiration_str)
    ...
    async def renew_subscription(self) -> bool:
        ...
        if result and "expirationDateTime" in result:
            ...
            logger.info("Webhook subscription renewed, expires=%s", self.expiration_time)
            return True
        else:
    ...
    async def _auto_renew_loop(self) -> None:
        ...
        while self._running:
            ...
            renew_in_seconds = time_until_expiry - (RENEW_BEFORE_EXPIRY_MINUTES * 60)
            ...
            if renew_in_seconds <= 0:
                ...
                logger.debug("Subscription renewal scheduled in %s seconds", renew_in_seconds)
                await asyncio.sleep(min(renew_in_seconds, 3600))  # Check at least hourly
    ...
    def validate_notification(
        ...
        if client_state != expected_client_state:
            logger.warning("Invalid clientState in notification: %s", client_state)
            return False
